...
//			jedec_id.manufacturer_id, jedec_id.memory_type, jedec_id.capacity);
...
//	flash_status = flash_write_data(address, tx_write_record_buffer,
//			sizeof(tx_write_record_buffer));
//	snprintf(uart_message, sizeof(uart_message), "Status: %d", flash_status);
//	HAL_UART_Transmit(&huart2, (uint8_t*) uart_message, strlen(uart_message),
//	HAL_MAX_DELAY);
//	flash_erase_sector(address);
//	flash_status = flash_read_data(address, rx_read_record_buffer,
//			sizeof(rx_read_record_buffer));
//
...
//				"0x%02X%s", rx_read_record_buffer[i], (i == 17) ? "\r\n" : ", ");
//	}

	snprintf(uart_message, sizeof(uart_message), "Value: %lu\r\n", address);
	HAL_UART_Transmit(&huart2, (uint8_t*) uart_message, strlen(uart_message),
	HAL_MAX_DELAY);
